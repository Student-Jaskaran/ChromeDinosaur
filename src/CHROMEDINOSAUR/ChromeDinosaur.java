...
public class ChromeDinosaur extends JPanel implements ActionListener,KeyListener
{
    ...
    public ChromeDinosaur()
 {
     ...
     //game timer
     gameLoop = new Timer(1000/60, this); // 1000/60 frames per 1000ms (1s),update
     gameLoop.start();
 ...
 void placeCactus()
 {
    ...
    double placeCactusChance = Math.random();//0-0.999999
     if(placeCactusChance > 0.90)//10% you get cactus3
     {
         Block cactus = new Block(cactusX, cactusY, cactus3Width, cactusHeight, cactus3Img);
         cactusArray.add(cactus);
     }
         else if(placeCactusChance > 0.70)//20% you get cactus2
     {
 ...
 {
     // dinosaur
     g.drawImage(dinosaur.img,dinosaur.x,dinosaur.y,dinosaur.width,dinosaur.height,null);// image create here
 ...
 boolean collision(Block a,Block b)
 {
     return a.x < b.x + b.width && // a's top left corner doesn't reach b's top right corner
             ...
             a.y + a.height > b.y;// a's top right corner passes  b's top left corner
 }
    @Override
    public void actionPerformed(ActionEvent e) // refresh the screen
    {
        move();
        repaint();
        if(gameOver)
...
//         System.out.println("JUMP!");
            ...
            if(gameOver)
            {
                //restart game by restarting conditions
                dinosaur.y =dinosaurY;
                dinosaur.img = dinosaurImg;
                velocityY = 0;
                cactusArray.clear();
